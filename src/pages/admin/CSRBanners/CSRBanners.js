...
const CSRBanners = () => {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        ...
        const response = await axios.get(`${apiUrl}/api/csr-banner`);
        const bannerData = response.data.banners;

        setBanners(bannerData);
      } catch (error) {
        console.error("Error fetching csr banners:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDeleteBanner = async (id, name) => {
    ...
    try {
      ...
      toast.success("csr banner deleted successfully!");
      
    } catch (error) {
      console.error("Error deleting csr banner:", error);
      setErrorMessage(
        error.response?.data?.message || "Failed to delete csr banner"
      );
      toast.error("Failed to delete csr banner");
    }
  };
  ...
  return (
    <AdminLayout>
      <div className="pages-headers ">
        <h2>
          CSR Banners
          <NavLink to="/admin/add/csr-banners" className="theme-cta">
                    ...
                    banners.map((banner) => (
                      <tr key={banner._id}>
                        ...
                        <td className="text-center">
                          <Link
                            to={`/admin/edit/csr-banners/${banner._id}`}
                            title="Edit"
                          >
