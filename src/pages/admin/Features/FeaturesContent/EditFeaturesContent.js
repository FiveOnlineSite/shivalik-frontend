...
setProjects(filtered);
  ...
  useEffect(() => {
    const fetchFeaturesContent = async () => {
      try {
        const apiUrl = process.env.REACT_APP_API_URL;
        const response = await axios.get(`${apiUrl}/api/feature-content/${id}`);
        const featuresContentData = response.data.FeaturesContent;
        setFeaturesContent(featuresContentData);

        console.log("featuresContent data", featuresContentData);

        setFormData({
          alt: featuresContentData.alt,
          title: featuresContentData.title,
          image: {
            file: featuresContentData.image?.[0]?.filename || "",
            filepath: featuresContentData.image?.[0]?.filepath || "",
          },
          project: featuresContentData.project?._id || featuresContentData.project || "",
        });
      } catch (error) {
  ...
  try {
    ...
    await axios.patch(`${apiUrl}/api/feature-content/${id}`, formDataToSend, 
        {
      headers: {
    ...
    toast.success("features Content updated successfully!");
  } catch (error) {
    console.error("Error updating features Content:", error);
    setErrorMessage(error.response?.data?.message || "An error occurred");
    toast.error("Failed to update features Content");
  } finally {
    setIsSubmitting(false);
  }
};
...
export default EditFeaturesContent;
